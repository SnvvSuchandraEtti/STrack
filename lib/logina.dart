...
import 'widgets.dart';
import 'signupa.dart'; // Import the registration screen
import 'user_state.dart';
import 'navbar1.dart'; // Ensure correct import

class SimpleLoginScreena extends StatefulWidget {
  final Function(String? email, String? password)? onSubmitted;

  const SimpleLoginScreena({this.onSubmitted, Key? key}) : super(key: key);

  @override
  State<SimpleLoginScreena> createState() => _SimpleLoginScreenState();
}

class _SimpleLoginScreenState extends State<SimpleLoginScreena> {
    ...
    if (validate()) {
      if (onSubmitted != null) {
        ...
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) => navbar(), // Ensure correct class name
          ),
        );
      }
    }
              ...
              children: [
                ...
                TextButton(
                  onPressed: () => Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (_) => SimpleRegisterScreena(),
                    ),
                  ),
                  child: RichText(
                    text: const TextSpan(
                  ...
                  child: TextButton.icon(
                    onPressed: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (_) => SimpleLoginScreena(), // Replace with your signup screen
                        ),
                      );
                    },
